---
title: "V1 APOE + Base Genome Benchmark Analysis"
...
cat("#! V1 APOE + Base Genome Benchmark Analysis\n\n")
...
I generated a custom genome using the methods described in [V1 Genome Benchmark Analysis](https://www.synapse.org/Synapse:syn73455492), however only human APOE was added to the base mouse genome. All samples were aligned to the APOE+ genome, and compared to the alignment with the base genome.

This benchmarking case tries to isolate the effect of adding the human APOE gene on alignment.
...
# Use V1 of the benchmark file, which does not include CLU tests
...
apoe_file <- synGet("syn61806211.1", downloadLocation = file.path("data", "tmp"))
counts_apoe <- read.table(apoe_file$path, sep = "\t", header = TRUE) |>
  select(-transcript_id.s.)
...
-   The APOE+ genome contains human genes, and the base genome doesn't
-   All human genes start with "ENSG" and all mouse genes start with "ENSM"
-   No genes are in the base genome that don't exist in the APOE+ genome
-   All samples are present in both matrices

```{r}
# Check for existence of human genes
human_genes <- setdiff(counts_apoe$gene_id, counts_base$gene_id)
mouse_genes <- intersect(counts_base$gene_id, counts_apoe$gene_id)

cat("Genes present in APOE+ genome that are not in base genome:", "\n",
...
stopifnot(length(setdiff(counts_base$gene_id, counts_apoe$gene_id)) == 0)
cat("All genes in base genome are present in APOE+ genome.", "\n")

# All samples present
stopifnot(all(colnames(counts_base) %in% colnames(counts_apoe)))
...
rownames(counts_apoe) <- counts_apoe$gene_id
counts_apoe <- counts_apoe[, colnames(counts_base)]

df_base <- melt(counts_base[mouse_genes, ], 
                variable.name = "sample", 
                value.name = "count")
df_apoe <- melt(counts_apoe[mouse_genes, ], 
                variable.name = "sample", 
                value.name = "count")
```
...
```{r}
pct <- 100 * sum(df_base$count == df_apoe$count) / nrow(df_base)
cat("Identical counts between matrices:", 
    sum(df_base$count == df_apoe$count), 
    "of", nrow(df_base), 
...
df_combined <- subset(df_base, df_base$count != df_apoe$count) |>
  merge(df_apoe, by = c("gene_id", "sample")) |>
  dplyr::rename(count_base = "count.x", count_apoe = "count.y") |>
  mutate(difference = count_base - count_apoe)

cat("Correlation between mismatches: R^2 =", 
    cor(df_combined$count_base, df_combined$count_apoe)^2, "\n")
...
Note that the 5XFAD study does not appear in this graph, indicating that counts were unaffected for both 5XFAD carriers and non-carriers.
...
ggplot(df_combined[diff_counts_gt1, ], 
  ...
  ylab("Difference from APOE+ genome")
```

## By genotype

Note that this graph appears to be dominated by the *APOE4-KI_homozygous* and *LOAD1* genotypes, but the highest differences come from both 3xTg-AD genotypes. This is odd because neither of those genotypes contain human APOE.
...
For each sample, how many genes have a count difference \> 1 between the base mouse genome and the APOE+ genome?
...
For each gene, how many samples have a count difference \> 1 between the base mouse genome and the APOE+ genome?
...
For each genotype, how many entries in the matrix (over all samples and genes) have a count difference \> 1 between the base mouse genome and the APOE+ genome?
...
For each genotype, how many unique genes have a count difference \> 1 in any sample between the base mouse genome and the APOE+ genome?
...
Note that from the graphs above, the only entries with \> 10 counts of difference between the base mouse genome and the APOE+ genome are entries with between 1,000 and 1,000,000 total counts, representing a margin of 1% to 0.001% difference.
...
Create a data frame with gene expression from humanized genes and mouse orthologs from the counts aligned with the APOE+ genome, and the mouse orthologs from the counts aligned with the base genome. Give human genes and their mouse orthologs recognizable gene names.
...
base_tmp <- melt(counts_base[names(id_map)[1], ],
                 variable.name = "sample", 
                 value.name = "count") |>
  mutate(gene_source = "Mouse Apoe (base genome)")

df_human <- melt(counts_apoe[names(id_map), ], 
                 variable.name = "sample", 
                 value.name = "count") |>
  mutate(gene_source = "Mouse Apoe (APOE+ genome)") |>
  rbind(base_tmp) |>
  merge(metadata, by.x = "sample", by.y = "specimenID_X")

df_human$gene_name <- id_map[df_human$gene_id]
df_human$gene_source[grepl("ENSG", df_human$gene_id)] <- "Human APOE"
```
...
Some mice from the *3xTg-AD_carrier*, *APOE4-KI_noncarrier*, and *Trem2-R47H_homozygous* genotypes, which do **not** contain hAPOE, had a small number of detected counts (\< 10 counts) for humanized APOE.
...
ggplot(df_human, aes(x = genotype, y = count+1, color = humanized_APOE)) +
  ...
  ggtitle("Gene expression of human and mouse APOE")
```
...
**What are the 8 genes with count differences \> 10?**
...
Expression differences of these genes appear to be direct swaps between two genes within the same sample:
...
-   Detection of human APOE appears consistent with each mouse genotype.
-   Detection of mouse *Apoe* is similar between the base mouse genome and the APOE+ genome, and expression levels are as expected across genotypes.
-   However, adding human APOE to the reference *does change detected counts for other mouse genes* as well:
    -   the majority of these genes are pseudogenes, mitochondrial, ribosomal, un-annotated, or non-coding RNAs
    -   Only a small number of genes have a count difference \> 10, and appear to be almost all non-coding, mitochondrial, ribosomal, pseudogenes, or unannotated genes.
-   These changes are so minor that it is safe to conclude that **adding human APOE to the genome doesn't significantly affect alignment**.
...
knitfile2synapse(file = "V1b_APOE_Benchmark_Analysis.Rmd",
                 owner = "syn73460864",
                 wikiName = "V1 APOE Genome Benchmark Analysis")
```
