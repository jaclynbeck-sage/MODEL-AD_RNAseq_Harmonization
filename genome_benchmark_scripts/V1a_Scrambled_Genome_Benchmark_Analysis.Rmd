---
title: "V1 Scrambled Genome Benchmark Analysis"
...
cat("#! V1 Scrambled Genome Benchmark Analysis\n\n")
...
I generated a custom genome using the methods described in [V1 Genome Benchmark Analysis](https://www.synapse.org/Synapse:syn73455492), however the sequences for the humanized genes (APOE, APP, MAPT, and PSEN1) were scrambled prior to adding them to the mouse genome. All samples were aligned to the scrambled genome, and compared to the alignment with the base genome.

This benchmarking case tests whether adding new sequences of any kind to the base genome has any effect on alignment.
...
# Use V1 of the base benchmark file, which does not include CLU tests
...
scrambled_file <- synGet("syn73452885.1", downloadLocation = file.path("data", "tmp"))
counts_scrambled <- read.table(scrambled_file$path, sep = "\t", header = TRUE) |>
  select(-transcript_id.s.)
...
-   The scrambled genome contains human genes, and the base genome doesn't
-   All human genes start with "ENSG" and all mouse genes start with "ENSM"
-   No genes are in the base genome that don't exist in the scrambled genome
-   All samples are present in both matrices

```{r}
human_genes <- setdiff(counts_scrambled$gene_id, counts_base$gene_id)
mouse_genes <- intersect(counts_base$gene_id, counts_scrambled$gene_id)

cat("Genes present in scrambled genome that are not in base genome:", "\n",
...
stopifnot(length(setdiff(counts_base$gene_id, counts_scrambled$gene_id)) == 0)
cat("All genes in base genome are present in scrambled genome.", "\n")

# All samples present
stopifnot(all(colnames(counts_base) %in% colnames(counts_scrambled)))
...
rownames(counts_scrambled) <- counts_scrambled$gene_id
counts_scrambled <- counts_scrambled[, colnames(counts_base)]

df_base <- melt(counts_base[mouse_genes, ], 
                variable.name = "sample", 
                value.name = "count")
df_scrambled <- melt(counts_scrambled[mouse_genes, ], 
                variable.name = "sample", 
                value.name = "count")
```
...
```{r}
pct <- 100 * sum(df_base$count == df_scrambled$count) / nrow(df_base)
cat("Identical counts between matrices:", 
    sum(df_base$count == df_scrambled$count), 
    "of", nrow(df_base), 
...
df_combined <- subset(df_base, df_base$count != df_scrambled$count) |>
  merge(df_scrambled, by = c("gene_id", "sample")) |>
  dplyr::rename(count_base = "count.x", count_scrambled = "count.y") |>
  mutate(difference = count_base - count_scrambled)

cat("Correlation between mismatches: R^2 =", 
    cor(df_combined$count_base, df_combined$count_scrambled)^2, "\n")
...
Note that the 3xTg-AD study does not appear in this graph, indicating that there are no differences \> 1 count for that study.
...
ggplot(df_combined[diff_counts_gt1, ], 
  ...
  ylab("Difference from scrambled genome")
```
...
Note that from the graphs above, the only entries with \> 10 counts of difference between the base mouse genome and the universal genome are entries with between 10,000 and 1,000,000 total counts, representing a margin of 0.1% to 0.001% difference.
...
Because the human gene sequences are scrambled, there should not be any detected counts for any human gene we added.
...
df_human <- melt(counts_scrambled[names(id_map), ], 
                 variable.name = "sample", 
                 value.name = "count") |>
  mutate(gene_source = "Mouse (scrambled)") |>
  rbind(base_tmp) |>
...
Expression differences of these genes appear to be direct swaps between two genes within the same sample, for only 2 samples:
...
-   No counts of scrambled human genes were detected in any sample.
-   However, adding the scrambled genes to the reference *does slightly change detected counts for other mouse genes* as well:
    -   the majority of these genes are pseudogenes, mitochondrial, ribosomal, un-annotated, or non-coding RNAs
    -   For each genotype, between 6 and 16 other mouse genes have a difference of \> 1 count in at least one sample
        -   Median expression change is \< 3 counts
        -   Max change is \< 6 counts, except for two `5XFAD_noncarrier` samples with larger changes in mitochondrial or unannotated genes, as shown above
-   These changes are so minor that it is safe to conclude that **simply adding more sequences to the genome doesn't significantly affect alignment**.
...
knitfile2synapse(file = "V1a_Scrambled_Genome_Benchmark_Analysis.Rmd",
                 owner = "syn73454887",
                 wikiName = "V1a Scrambled Genome Benchmark Analysis")
```
