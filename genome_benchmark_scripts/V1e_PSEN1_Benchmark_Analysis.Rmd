---
title: "V1 PSEN1 + Base Genome Benchmark Analysis"
...
cat("#! V1 PSEN1 + Base Genome Benchmark Analysis\n\n")
...
I generated a custom genome using the methods described in [V1 Genome Benchmark Analysis](https://www.synapse.org/Synapse:syn73455492), however only human PSEN1 was added to the base mouse genome. All samples were aligned to the PSEN1+ genome, and compared to the alignment with the base genome.

This benchmarking case tries to isolate the effect of adding the human PSEN1 gene on alignment.
...
psen1_file <- synGet("syn61806417.1", downloadLocation = file.path("data", "tmp"))
counts_psen1 <- read.table(psen1_file$path, sep = "\t", header = TRUE) |>
  select(-transcript_id.s.)
...
-   The PSEN1+ genome contains human genes, and the base genome doesn't
-   All human genes start with "ENSG" and all mouse genes start with "ENSM"
-   No genes are in the base genome that don't exist in the PSEN1+ genome
-   All samples are present in both matrices

```{r}
# Check for existence of human genes
human_genes <- setdiff(counts_psen1$gene_id, counts_base$gene_id)
mouse_genes <- intersect(counts_base$gene_id, counts_psen1$gene_id)

cat("Genes present in PSEN1+ genome that are not in base genome:", "\n",
...
stopifnot(length(setdiff(counts_base$gene_id, counts_psen1$gene_id)) == 0)
cat("All genes in base genome are present in PSEN1+ genome.", "\n")

# All samples present
stopifnot(all(colnames(counts_base) %in% colnames(counts_psen1)))
...
rownames(counts_psen1) <- counts_psen1$gene_id
counts_psen1 <- counts_psen1[, colnames(counts_base)]

df_base <- melt(counts_base[mouse_genes, ], 
                variable.name = "sample", 
                value.name = "count")
df_psen1 <- melt(counts_psen1[mouse_genes, ], 
                variable.name = "sample", 
                value.name = "count")
```
...
```{r}
pct <- 100 * sum(df_base$count == df_psen1$count) / nrow(df_base)
cat("Identical counts between matrices:", 
    sum(df_base$count == df_psen1$count), 
    "of", nrow(df_base), 
...
df_combined <- subset(df_base, df_base$count != df_psen1$count) |>
  merge(df_psen1, by = c("gene_id", "sample")) |>
  dplyr::rename(count_base = "count.x", count_psen1 = "count.y") |>
  mutate(difference = count_base - count_psen1)

cat("Correlation between mismatches: R^2 =", 
    cor(df_combined$count_base, df_combined$count_psen1)^2, "\n")
...
ggplot(df_combined[diff_counts_gt1, ], 
  ...
  ylab("Difference from PSEN1+ genome")
```
...
For each sample, how many genes have a count difference \> 1 between the base mouse genome and the PSEN1+ genome?
...
For each gene, how many samples have a count difference \> 1 between the base mouse genome and the PSEN1+ genome?
...
For each genotype, how many entries in the matrix (over all samples and genes) have a count difference \> 1 between the base mouse genome and the PSEN1+ genome?
...
For each genotype, how many unique genes have a count difference \> 1 in any sample between the base mouse genome and the PSEN1+ genome?
...
Note that from the graphs above, the only entries with \> 10 counts of difference between the base mouse genome and the PSEN1+ genome are entries with between 1,000 and 1,000,000 total counts, representing a margin of 1% to 0.001% difference.
...
Create a data frame with gene expression from humanized genes and mouse orthologs from the counts aligned with the PSEN1+ genome, and the mouse orthologs from the counts aligned with the base genome. Give human genes and their mouse orthologs recognizable gene names.

```{r message=FALSE}
id_map <- c("ENSMUSG00000019969" = "PSEN1",
            "ENSG00000080815" = "PSEN1")

base_tmp <- melt(counts_base[names(id_map)[1], ],
                 variable.name = "sample", 
                 value.name = "count") |>
  mutate(gene_source = "Mouse Psen1 (base genome)")

df_human <- melt(counts_psen1[names(id_map), ], 
                 variable.name = "sample", 
                 value.name = "count") |>
  mutate(gene_source = "Mouse Psen1 (PSEN1+ genome)") |>
  rbind(base_tmp) |>
  merge(metadata, by.x = "sample", by.y = "specimenID_X")

df_human$gene_name <- id_map[df_human$gene_id]
df_human$gene_source[grepl("ENSG", df_human$gene_id)] <- "Human PSEN1"
```
...
One sample of the *3xTg-AD_noncarrier* genotype expresses 1 count of hPSEN1, which can be considered trivial, but one sample of the *5XFAD_noncarrier* genotype expresses 40 counts of hPSEN1. This sample has already been determined to be mis-genotyped or contaminated and can be ignored.
...
**What are the 4 genes with count differences \> 10?**
...
-   Detection of human PSEN1 appears consistent with each mouse genotype, except for one mis-genotyped or contaminated *5XFAD_noncarrier* sample.
-   Detection of mouse *Psen1* is similar between the base mouse genome and the PSEN1+ genome, and expression levels are as expected across genotypes.
-   However, adding human PSEN1 to the reference *does change detected counts for a small number of mouse genes* as well:
    -   the majority of these genes are pseudogenes, mitochondrial, ribosomal, un-annotated, or non-coding RNAs
-   These changes are so minor that it is safe to conclude that **adding human PSEN1 to the genome doesn't significantly affect alignment**.
...
knitfile2synapse(file = "V1e_PSEN1_Benchmark_Analysis.Rmd",
                 owner = "syn73462195",
                 wikiName = "V1 PSEN1 + Base Genome Benchmark Analysis")
```
